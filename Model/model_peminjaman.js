...
class Model_peminjaman {
  static async getAll() {
    return new Promise((resolve, reject) => {
      connection.query('SELECT * FROM peminjaman JOIN lab ON peminjaman.id_lab = lab.id_lab JOIN pengguna ON peminjaman.id_pengguna = pengguna.id_pengguna ', (err, rows) => {
        if (err) {
          reject(err);
        } else {
          resolve(rows);
  ...
  static async Update(id, data) {
    return new Promise((resolve, reject) => {
      connection.query('UPDATE Peminjaman SET ? WHERE id_peminjaman = ?', [data, id], (err, result) => {
        if (err) {
          reject(err);
        } else {
          resolve(result);
